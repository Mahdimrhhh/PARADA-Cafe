...
//#region node_modules/.nitro/vite/services/ssr/assets/menu-BUSaKwNc.js
...
async function seedIfEmpty() {
	...
	const { seedMenu } = await import("./seed.server-Dvy9CHBe.mjs");
	await seedMenu(sql);
	await ensurePinHashed(sql);
}
...
var verifyPin = createServerFn({ method: "POST" }).validator(object({ pin: string().min(1).max(40) })).handler(verifyPin_createServerFn_handler, async ({ data }) => {
	const sql = await getSql();
	await seedIfEmpty();
...
var updateItem = createServerFn({ method: "POST" }).validator(itemInput.extend({ id: number().int().positive() })).handler(updateItem_createServerFn_handler, async ({ data }) => {
	const db = await await getSql();
	await db`
      update items set
...
var deleteItem = createServerFn({ method: "POST" }).validator(object({ id: number().int().positive() })).handler(deleteItem_createServerFn_handler, async ({ data }) => {
	const db = await await getSql();
...
var updateCategory = createServerFn({ method: "POST" }).validator(categoryInput.extend({ id: number().int().positive() })).handler(updateCategory_createServerFn_handler, async ({ data }) => {
	const db = await await getSql();
	await db`
...
var deleteCategory = createServerFn({ method: "POST" }).validator(object({ id: number().int().positive() })).handler(deleteCategory_createServerFn_handler, async ({ data }) => {
	const sql = await getSql();
	if (((await sql`
